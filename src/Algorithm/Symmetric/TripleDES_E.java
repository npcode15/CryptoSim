...
import Misc.TripleDES;

public class TripleDES_E extends JFrame implements ActionListener
{
	...
	public TripleDES_E()
	{
		...
		saveToFile=new JButton("Save Encrypted-Text to a File");			// Save To Mail 
		...
		keyinput.setToolTipText("Minimum 8 characters required".toUpperCase());
		add(keyinput);
	...
	{ 
		...
		{   
			if(keyselect.isSelected())
			{
				...
				try
				{
					stream = new FileOutputStream("PrivateKey[Triple_DES]"+".txt"); 
			...
			else
			{
		        ...
		        m.update(encryptionKey.getBytes(),0,encryptionKey.length());
		        encryptionKey+=new BigInteger(1,m.digest()).toString(16);
	...
	public static String encrypt (String plaintext, String encryptionKey) throws UnsupportedEncodingException, InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, InvalidKeySpecException
	{
		...
		String encryptedString = base64encoder.encodeToString(encryptedText);
