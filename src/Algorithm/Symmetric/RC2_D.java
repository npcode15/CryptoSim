...
public class RC2_D extends JFrame implements ActionListener
{
	...
	{	
		...
		{  
			...
			{
				if((decryptionKey.length()%8)!=0 && decryptionKey.length()<16 && decryptionKey.length()>8) 
					while((decryptionKey.length()%16)!=0)
						decryptionKey+=" ";

				if((decryptionKey.length()%8)!=0 && decryptionKey.length()>16)
				...
				plaintext = decrypt(decryptionKey.getBytes(), IV.getBytes(), ciphertext);
	...
	public static String decrypt(byte[] key, byte[] iv, String encrypted) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException
	{
