...
public class TripleDES_D extends JFrame implements ActionListener
{
	...
	{	
		...
		{  
			...
			{
				if((decryptionKey.length()%8)!=0 && decryptionKey.length()>8)
					decryptionKey=decryptionKey.substring(0,8);

				MessageDigest m=MessageDigest.getInstance("MD5");
				m.update(decryptionKey.getBytes(),0,decryptionKey.length());
				decryptionKey+=new BigInteger(1,m.digest()).toString(16);
	...
	{
		byte[] keyAsBytes = decryptionKey.getBytes("UTF-8");
		...
		byte[] encryptedText = base64decoder.decode(cipherText);
		byte[] plainText = cipher.doFinal(encryptedText);
		String decryptedText= byte2String(plainText);//new Base64().encodeToString(plainText);
