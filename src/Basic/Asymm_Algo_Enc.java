...
import Algorithm.Asymmetric.ElGamal_E;
import Algorithm.Asymmetric.Paillier_E;
import Algorithm.Asymmetric.RSA_E;

public class Asymm_Algo_Enc extends JFrame implements ActionListener 
{
	...
	public void actionPerformed(ActionEvent evt)
	{
		...
		{
			new ElGamal_E().setVisible(true);
			this.dispose();
		}

		if(clicked.equals(publicalgo[2]))
		{
			new RSA_E().setVisible(true);
			this.dispose();
		}

		if(clicked.equals(publicalgo[1]))
		{
			new Paillier_E().setVisible(true);
			this.dispose();
		}
	}
}